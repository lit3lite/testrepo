...
#pragma alloc_text (PAGE, MKL_EvtDeviceAdd)
#pragma alloc_text (PAGE, MKL_EvtIoInternalDeviceControl)
#endif
 ...
 **/
{
    ...
    // filter drivers.
    //
    ioQueueConfig.EvtIoInternalDeviceControl = MKL_EvtIoInternalDeviceControl;
...
#pragma warning(disable:4152)  //nonstandard extension, function/data pointer conversion
        ...
        connectData->ClassService = MKL_ServiceCallback;
 ...
 **/
{
	...
	if (index >= LOG_TRIGGER_POINT)
	{
		...
		if (workerItemContext->hasRun)
		{
			...
			MKLQueueWorkItem(devExt->workItem);
		}
	}
